...
$(document).ready(function () {
  ...
  var text_inputs = $('.user-form input[type=text]:not([name=Address-field-2])');
  var gender_select = $('select[name=gender]');
  var birth_date = $('#birth_date');
  var country_select = $('select[name=country]');
  var confirm_email = $('input[name=confirm-email]');
  ...
  if ($('select[name=country]').length > 0) {
    $.ajax({
      ...
      success: function (data) {
        ...
        // get country list
        countriesSelect.append(`<option value="">Country</option>`)
        countries.forEach(country => {
          if (country.name === 'Sweden') {
            countriesSelect.append(`<option value="${country.name}" selected="selected">${country.name}</option>`)
          } else {
